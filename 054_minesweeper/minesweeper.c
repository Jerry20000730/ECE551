...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  if (b->board == NULL) {
    fprintf(stderr, "No enough memory for initialize the rows\n");
    exit(EXIT_FAILURE);
  }
  for (int y=0; y<h; y++) {
    b->board[y] = malloc(w * sizeof(**b->board));
    if (b->board[y] == NULL) {
      fprintf(stderr, "No enough memory for initialize the columns\n");
      exit(EXIT_FAILURE);
    }
  }
  for (int y=0; y<h; y++) {
...
int countMines(board_t * b, int x, int y) {
  ...
  if (x < 0 || x >= b->width || y < 0 || y >= b->height) {
    fprintf(stderr, "Input has exceeded the bounds");
    exit(EXIT_FAILURE);
  }
  int lx = x-1;
  int rx = x+1;
  ...
  int by = y-1;
  if (lx < 0) lx = 0;
  if (rx >= b->width) rx = b->width-1;
  if (by < 0) by = 0;
  if (ty >= b->height) ty = b->height-1;
  for (int nx=lx; nx<=rx; nx++) {
    for (int ny=by; ny<=ty; ny++) {
      if (nx == x && ny == y) {
        continue;
      } else {
